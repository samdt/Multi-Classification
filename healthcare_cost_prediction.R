...
Claims = read.csv("C:\\Users\\Vaibhav\\Desktop\\Chaps\\ML\\Assignments\\healthcare-master\\healthcare-master\\ClaimsData.csv")
...
#The baseline accuracy is the sum of the diagonal, the observations that
#were classified correctly, divided by the total number of observations in our test set.

#Accuracy= (sum(diag(table(ClaimsTest$bucket2009,ClaimsTest$bucket2008))))/nrow(ClaimsTest)
...
#Penalty matrix is also called as loss matrix - it always has 0s on the diagonal and +ve off - diagonal elements. 
#We can pass this loss matrix as an a parameter in rpart command for a gini-based 
# classification algorithm, and it will help in splitting the tree to minimize the loss. 
...
# Building the model with each of the complexity parameters in the cpGrid
